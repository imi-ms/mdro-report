...
/** create a copy of the map with the given key's value transformed by transformer, similar to DataFrame.update **/
fun <K, V> Map<K, V>.update(key: K, transformer: (V) -> V): Map<K, V> {
    return this.mapValues { (k, v) -> if (k == key) transformer(v) else v }
}

private val germanDatePattern = DateTimeFormatter.ofPattern("dd.MM.yyyy")
fun toGermanDate(v: String): String = LocalDateTime.parse(v).toLocalDate().format(germanDatePattern)
