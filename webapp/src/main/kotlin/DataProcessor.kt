...
object DataProcessor {
    ...
    fun countMRSANosokomial(caseList: String) =
        WebappComponents.getMRSACSV(caseList).filter { it["nosokomial?"] == "true" }.size

    fun countMRSAImported(caseList: String) =
        WebappComponents.getMRSACSV(caseList).filter { it["nosokomial?"] != "true" }.size

    fun countMRGN3Cases(caseList: String) =
        WebappComponents.getMRGACSV(caseList).filter { it["Piperacillin und Tazobactam Ergebnis"] == "MRGN3" }.size

    fun countMRGN4Cases(caseList: String) =
        WebappComponents.getMRGACSV(caseList).filter { it["Piperacillin und Tazobactam Ergebnis"] == "MRGN4" }.size
