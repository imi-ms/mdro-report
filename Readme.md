...
- A directory where the files should saved has to be selected 
in order to start the program.
- To test the functionality of the MREReport tool to compare the 
results of different years, a range of years can be selected. 
The generated cases will lie between those years (so the upper year is not included)  

Right now the data should only be used to test the MREReport tool. 
